...
The version of hcdocs that you just built only uses static files which are all contained in the `site/`, directory so you will be able to host it from pretty much anywhere. You can view a live version of the documentation at https://h.cash.
...
```bash
$ docker run -d --rm -p <local port>:80 hcash/hcdocs:latest
```
...
When translations are added/updated in [Transifex](https://www.transifex.com/hcash/), pull the updates:
